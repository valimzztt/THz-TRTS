...
def differential_reflectivity(E_Ref_windowed, E_Pump_windowed, time_axis):
    """
    Calculate the differential reflectivity (dR/R) between E_Ref and E_Pump.

    Parameters:
    - E_Ref_windowed: 2D array of reference electric field (time-domain).
    - E_Pump_windowed: 2D array of pump electric field (time-domain).
    - time_axis: 1D array of time points corresponding to the signals.

    Returns:
    - dR: Differential reflectivity as a 2D array.
    - freqs: Frequency axis corresponding to the differential reflectivity.
    """
    # Compute the time sample spacing
    time_sample = time_axis[1] - time_axis[0]

    freqs = np.fft.fftfreq(len(time_axis), d=time_sample)
    freqs = freqs[:len(freqs) // 2]

    E_Ref_fft = np.fft.fft(E_Ref_windowed, axis=0)
    E_Pump_fft = np.fft.fft(E_Pump_windowed, axis=0)

    E_Ref_fft = E_Ref_fft[:len(freqs),]
    E_Pump_fft = E_Pump_fft[:len(freqs),]

    delta_E = E_Pump_fft - E_Ref_fft
...
if __name__ == "__main__":
    ...
    def update_plot(self):
        ...
        dR, freqs = differential_reflectivity(E_Ref_windowed, E_Pump_windowed, time_axis)
        ...
        axs[1].set_title(f"Differential Reflectivity, pump delay= {pump_delay} ps")
        axs[1].plot(freqs, dR, label="dR")
