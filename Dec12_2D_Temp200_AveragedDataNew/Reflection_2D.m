...
Selection = 3;     % Start menu selection
...
function err = fitfun(c)
    ...
    eps_lorentz = 1i*bsxfun(@rdivide, wpL.^2,  eps0*( 1i*(w0^2 - (w*Freq(ffl)).^2) + w*Freq(ffl)*gam) );
...
eps_int = eps_int * dim;
sigma_dark_int = bsxfun(@times, 1i*eps0*w*Freq_int, epsi - eps_int);
n_int = sqrt(eps_int);
